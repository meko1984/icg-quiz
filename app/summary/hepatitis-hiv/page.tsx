...
export default function HepatitisHiv() {
  return <StudyPage title="肝炎とHIVの比較"><div className={s.page}>
    ...
    <section id="risk" className={s.section}><h2>「曝露後に感染する率」とは</h2><p>以下は主に<strong>感染源となる血液への経皮曝露（針刺しなど）</strong>の報告です。感染者全体の割合や発症率ではありません。曝露量、針の深さ、感染源のウイルス量、本人の免疫、予防処置で変わります。</p>
      <div className={s.tableScroll} role="region" aria-label="感染率の比較表（横スクロール）" tabIndex={0}><table className={s.table}><caption>講義の学習値と、条件を明記した報告値</caption><thead><tr><th scope="col">ウイルス</th><th scope="col">講義・クイズ</th><th scope="col">報告値と条件</th></tr></thead><tbody>
        ...
        <tr><th scope="row">HIV</th><td><b>約0.3%</b><small>従来の学習値</small></td><td>米国2025ガイドライン：HIV感染血液への経皮曝露は平均<b>0.23%</b>（95%信頼区間0〜0.46%）、粘膜曝露は約<b>0.09%</b>。PEP後の失敗率ではありません。[7]</td></tr>
    ...
    <section className={s.sources}><h2>出典・確認日</h2><p>2026年9月9日確認。講義値は第1回資料72ページの書き込みを含む内容に対応しています。</p><ol>
